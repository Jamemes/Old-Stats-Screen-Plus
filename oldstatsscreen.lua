if string.lower(RequiredScript) == "lib/managers/hud/newhudstatsscreen" then
		...
		if stage_data then
			...
			if job_data then
				...
				if Global.game_settings.one_down then
					...
					local modifier_string = one_down_string
	...
	function HUDStatsScreen:extended_inventory()
		...
		if _G.HUDStatsScreenOptions.settings.Always_Show_Body_Bags or is_whisper_mode then
			body_bags_panel = extended:add_top(self._right:panel({name = "body_bags_panel", h = 19, w = self._right:w() / 1.2}))
		end
		
		if is_whisper_mode then
			if managers.job:is_level_ghostable(managers.job:current_level_id()) or lvl == "welcome_to_the_jungle_2" or lvl == "mallcrasher" then
				pager_panel = extended:add_top(self._right:panel({name = "pager_panel", h = 19, w = self._right:w() / 1.2}), 5)
			end
		end
		
		if _G.HUDStatsScreenOptions.settings.Always_Show_Kills or kill_requirements then
			kill_panel = extended:add_top(self._right:panel({name = "kill_panel", h = 19, w = self._right:w() / 1.2}), 5)
		end
		
		if _G.HUDStatsScreenOptions.settings.Always_Show_Accuracy or accuracy_requirements then
...
if string.lower(RequiredScript) == "lib/managers/localizationmanager" then
	_G.HUDStatsScreenOptions = _G.HUDStatsScreenOptions or {}
	HUDStatsScreenOptions.options_menu = "HUDStatsScreenOptions_menu"
	HUDStatsScreenOptions.interface_menu = "HUDStatsScreenOptions_interface_menu"
	HUDStatsScreenOptions.ModPath = ModPath
	HUDStatsScreenOptions.SaveFile = HUDStatsScreenOptions.SaveFile or SavePath .. "HUDStatsScreenOptions.txt"
	HUDStatsScreenOptions.ModOptions = HUDStatsScreenOptions.ModPath .. "menus/modoptions.txt"
	HUDStatsScreenOptions.settings = HUDStatsScreenOptions.settings or {}

	function HUDStatsScreenOptions:Reset()
		self.settings = {
			Always_Show_Body_Bags = false,
			Always_Show_Accuracy = false,
			Always_Show_Kills = false
		}
		self:Save()
	end

	function HUDStatsScreenOptions:Load()
	...
	function HUDStatsScreenOptions:Save()
	...
	HUDStatsScreenOptions:Load()

	local data = LocalizationManager.text
	function LocalizationManager:text(string_id, ...)
		return
		string_id == "menu_OP_title" 					and "Old stats screen +" or
		string_id == "menu_always_show_accuracy_title" 	and "Always show accuracy" or
		string_id == "menu_always_show_kills_title" 	and "Always show kill counter" or
		string_id == "menu_always_show_body_bags_title" and "Always show body bags amount" or
		string_id == "menu_accuracy" 					and "Accuracy" or
		string_id == "menu_kills" 						and "Total Kills" or
		data(self, string_id, ...)
	end

	Hooks:Add("MenuManagerSetupCustomMenus", "HUDStatsScreenOptionsOptions", function(menu_manager, nodes)
		MenuHelper:NewMenu(HUDStatsScreenOptions.options_menu)
		MenuHelper:NewMenu(HUDStatsScreenOptions.interface_menu)
	end)
	Hooks:Add("MenuManagerPopulateCustomMenus", "HUDStatsScreenOptionsOptions", function(menu_manager, nodes)
		
		MenuCallbackHandler.HUDStatsScreenOptions_menu_Always_Show_Body_Bags_callback = function(self, item)
			HUDStatsScreenOptions.settings.Always_Show_Body_Bags = item:value() == "on" and true or false
			HUDStatsScreenOptions:Save()
		end
		MenuHelper:AddToggle({
			id = "HUDStatsScreenOptions_menu_Always_Show_Body_Bags_callback",
			title = "menu_always_show_body_bags_title",
			callback = "HUDStatsScreenOptions_menu_Always_Show_Body_Bags_callback",
			value = HUDStatsScreenOptions.settings.Always_Show_Body_Bags,
			menu_id = HUDStatsScreenOptions.interface_menu,  
		})
		
		MenuCallbackHandler.HUDStatsScreenOptions_menu_Always_Show_Accuracy_callback = function(self, item)
			HUDStatsScreenOptions.settings.Always_Show_Accuracy = item:value() == "on" and true or false
			HUDStatsScreenOptions:Save()
		end
		MenuHelper:AddToggle({
			id = "HUDStatsScreenOptions_menu_Always_Show_Accuracy_callback",
			title = "menu_always_show_accuracy_title",
			callback = "HUDStatsScreenOptions_menu_Always_Show_Accuracy_callback",
			value = HUDStatsScreenOptions.settings.Always_Show_Accuracy,
			menu_id = HUDStatsScreenOptions.interface_menu,  
		})
		
		MenuCallbackHandler.HUDStatsScreenOptions_menu_Always_Show_Kills_callback = function(self, item)
			HUDStatsScreenOptions.settings.Always_Show_Kills = item:value() == "on" and true or false
			HUDStatsScreenOptions:Save()
		end
		MenuHelper:AddToggle({
			id = "HUDStatsScreenOptions_menu_Always_Show_Kills_callback",
			title = "menu_always_show_kills_title",
			callback = "HUDStatsScreenOptions_menu_Always_Show_Kills_callback",
			value = HUDStatsScreenOptions.settings.Always_Show_Kills,
			menu_id = HUDStatsScreenOptions.interface_menu,  
		})
	end)

	Hooks:Add("MenuManagerBuildCustomMenus", "HUDStatsScreenOptionsOptions", function(menu_manager, nodes)
		nodes[HUDStatsScreenOptions.interface_menu] = MenuHelper:BuildMenu(HUDStatsScreenOptions.interface_menu)
		MenuHelper:AddMenuItem(nodes["blt_options"], HUDStatsScreenOptions.interface_menu, "menu_OP_title", "")
	end)
end
